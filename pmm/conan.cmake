set(PMM_CONAN_MIN_VERSION 2.28.0 CACHE INTERNAL "Minimum Conan version we support")
set(PMM_CONAN_MAX_VERSION 2.31.99 CACHE INTERNAL "Maximum Conan version we support")
...
function(_pmm_ensure_conan)
    ...
    if (py3_exe)
        ...
        _pmm_log(VERBOSE "No Python 3 candidate found with quickfind. We'll check again with FindPython.")
    endif ()
    if (PMM_CONAN_EXECUTABLE)
        return()
    endif ()
    ...
    if (Python_FOUND)
        _pmm_get_conan_venv("Python 3" "${Python_EXECUTABLE}")
    else ()
        _pmm_log(VERBOSE "No Python candidate found.")
    endif ()
    if (NOT py3_exe AND NOT Python_FOUND)
...
function(_pmm_conan_profile_detect_default)
    _pmm_log(STATUS "Checking if a default profile exists")
    _pmm_exec("${PMM_CONAN_EXECUTABLE}" profile path default NO_EAT_OUTPUT)
    if(NOT ${_PMM_RC} EQUAL "0")
        _pmm_log(STATUS "The default profile doesn't exist, detecting it.")
        _pmm_exec("${PMM_CONAN_EXECUTABLE}" profile detect)
    endif()
endfunction()
...
function(_pmm_conan_get_settings out)
    ...
    elseif (comp_id STREQUAL Clang)
        ...
        set(use_version "${CMAKE_MATCH_1}")
        if (use_version VERSION_GREATER_EQUAL 8.0)
            string(REGEX REPLACE "^([0-9]+)\\..*" "\\1" use_version "${use_version}")
        endif ()
        _pmm_log(DEBUG "Using compiler.version=${use_version}")
        list(APPEND ret "compiler.version=${use_version}")
    ...
    if (NOT ARG_SETTINGS MATCHES ";?cppstd=")
        ...
        if(CMAKE_CXX_EXTENSIONS)
            if(compiler STREQUAL "msvc")
                ...
                set(${cxx_standard} "gnu${CMAKE_CXX_STANDARD}")
            endif()
        endif()
        if(cxx_standard)
            list(APPEND ret compiler.cppstd=${cxx_standard})
        endif ()
    endif ()
...
function(_pmm_conan_create_profile ${_build_type})
    ...
    set(profile_lines "# Profile generated by PMM ${PMM_VERSION}\n# https://github.com/AnotherFoxGuy/pmm\n\n[settings]")
    ...
    list(APPEND profile_lines "" "[conf]")
    list(APPEND profile_lines "tools.cmake.cmaketoolchain:generator=\"${CMAKE_GENERATOR}\"\n")
    set(_compilers_list "")
    if (CMAKE_C_COMPILER)
        list(APPEND _compilers_list "\"c\":\"${CMAKE_C_COMPILER}\"")
    endif ()
    if (CMAKE_CXX_COMPILER)
        list(APPEND _compilers_list "\"cpp\":\"${CMAKE_CXX_COMPILER}\"")
    endif ()
    if (CMAKE_RC_COMPILER)
        list(APPEND _compilers_list "\"rc\":\"${CMAKE_RC_COMPILER}\"")
    endif ()

    string(REPLACE ";" "," _compilers_list "${_compilers_list}")
    string(APPEND profile_lines "tools.build:compiler_executables={${_compilers_list}}\n")
...
function(_pmm_conan_run_install _build_type _generator_name)
    ...
    if (ARG_HOST_PROFILE)
        set(conan_args --profile:host "${ARG_HOST_PROFILE}")
    else ()
        if (NOT ARG_NO_DEFAULT_PROFILE)
            _pmm_conan_profile_detect_default()
            set(conan_args --profile:host default)
        else ()
            _pmm_conan_create_profile(${_build_type})
            set(conan_args --profile:host "${profile_file}")
        endif ()
    endif ()

    if (ARG_BUILD_PROFILE)
        set(conan_args --profile:build "${ARG_BUILD_PROFILE}")
    else ()
        set(conan_args --profile:build default)
    endif ()

    list(APPEND conan_args -of "${PMM_DIR}/conan/")
    list(APPEND conan_args --generator ${_generator_name} --build ${ARG_BUILD} -s build_type=${_build_type})
...
macro(_pmm_conan_install)
    set(__conan_inc "${PMM_DIR}/conan/conan_cmakedeps_paths.cmake")
    if (CONAN_EXPORTED)
        ...
        if (CMAKE_CONFIGURATION_TYPES AND NOT CMAKE_BUILD_TYPE AND NOT ARGUMENTS_BUILD_TYPE)
            _pmm_conan_run_install("Debug" "CMakeConfigDeps")
            _pmm_conan_run_install("Release" "CMakeConfigDeps")
        else ()
            if (NOT CMAKE_BUILD_TYPE)
                _pmm_log("WARNING: CMAKE_BUILD_TYPE was not set explicitly. We'll install your dependencies as 'Release'")
                set(CMAKE_BUILD_TYPE Release)
            endif ()
            _pmm_conan_run_install("${CMAKE_BUILD_TYPE}" "CMakeConfigDeps")
        endif ()
    endif ()
